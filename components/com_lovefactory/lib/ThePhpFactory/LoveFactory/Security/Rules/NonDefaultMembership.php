...
class NonDefaultMembership extends Rule
{
    public function authorize(\JUser $user)
    {
        ...
        if ($profile->hasDefaultMembership()) {
            $exception = new Redirect(
                \FactoryText::_('restriction_nondefaultmembership_error_message')
            );
            $exception->setRedirect(\JRoute::_('index.php?option=com_lovefactory&view=memberships'));
