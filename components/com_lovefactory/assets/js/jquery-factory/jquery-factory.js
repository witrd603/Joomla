...
(function (E, B) {
    ...
    var t = E.document, c = function () {
        ...
        var b = function (j, s) {
            ...
            }, d = E.jQueryFactory, e = E.$, f, h = /^(?:[^<]*(<[\w\W]+>)[^>]*$|#([\w\-]+)$)/, l = /\S/, k = /^\s+/, o = /\s+$/, x = /\W/, r = /\d/, A = /^<(\w+)\s*\/?>(?:<\/\1>)?$/,
        ...
        b.fn = b.prototype = {
            init: function (j,
                ...
                } else return !s || s.jQueryFactory ? (s || f).find(j) : b(s).find(j); else if (b.isFunction(j))return f.ready(j);
                if (j.selector !== B) {
                    this.selector = j.selector;
                    this.context = j.context
                }
                return b.makeArray(j, this)
            }, selector: "", jQueryFactory: "1.4.4", length: 0, size: function () {
                return this.length
            },
            toArray: function () {
    ...
    c.fn.extend({
        ...
        not: function (a) {
            ...
            if (c.isArray(a)) {
                ...
                if (h && a.length) {
                    ...
                    for (; h && h.ownerDocument && h !== b;) {
                        for (l in k) {
                            e = k[l];
                            if (e.jQueryFactory ? e.index(h) > -1 : c(h).is(e))d.push({selector: l, elem: h, level: o})
                        }
                        h =
                            h.parentNode;
                        o++
            ...
            return c.inArray(a.jQueryFactory ? a[0] : a, this)
        }, add: function (a, b) {
    ...
    var za = / jQueryFactory\d+="(?:\d+|null)"/g, $ = /^\s+/, Aa = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/ig, Ba = /<([\w:]+)/, db = /<tbody/i, eb = /<|&#?\w+;/, Ca = /<(?:script|object|embed|option|style)/i, Da = /checked\s*(?:[^=]|=\s*.checked.)/i, fb = /\=([^="'>\s]+\/)>/g, P = {
        option: [1,
    ...
    c.extend({
        ...
        getScript: function (a, b) {
            ...
            if (c.isArray(a) || a.jQueryFactory)c.each(a, function () {
